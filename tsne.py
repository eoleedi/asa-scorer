    ...
    def __init__(self, set):
        ...
        if set == "train":
            self.utt_label = torch.tensor(
                np.load("data/tr_label_utt.npy"), dtype=torch.float
            )
        elif set == "test":
            self.utt_label = torch.tensor(
                np.load("data/te_label_utt.npy"), dtype=torch.float
            )
        self.paths = paths
...
def draw_fig(X_tsne, y, dataset_type, fig_path):
    ...
    scatter = plt.scatter(
        X_tsne[:, 0], X_tsne[:, 1], c=y, alpha=0.5, cmap=plt.cm.Spectral
    )
    # 加入圖例
    legend1 = plt.legend(
...
if __name__ == "__main__":
    ...
    X_tsne = manifold.TSNE(
        n_components=2, init="random", random_state=5, perplexity=30, verbose=1
    ).fit_transform(extract_feat_tensor.numpy())
